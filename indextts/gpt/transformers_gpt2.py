...
# SequenceSummary已在新版transformers中移除，创建兼容版本
class SequenceSummary(nn.Module):
    """
    Compute a single vector summary of a sequence hidden states.
    兼容版本的SequenceSummary类，用于替代transformers中已移除的版本
    """
    def __init__(self, config):
        ...
        if self.summary_type == 'attn':
            ...
            # We can use the multi-head attention module of pytorch >=1.1.0
            self.summary = nn.MultiheadAttention(config.hidden_size, config.num_attention_heads, dropout=config.summary_dropout)
        elif self.summary_type == 'attn_proj':
            self.summary = nn.MultiheadAttention(config.hidden_size, config.num_attention_heads, dropout=config.summary_dropout)
            self.summary_proj = nn.Linear(config.hidden_size, config.hidden_size)
        elif self.summary_type in ['last', 'first', 'mean', 'cls_index']:
            self.summary = nn.Identity()
        else:
            raise ValueError(f"Unsupported summary type: {self.summary_type}")

        self.activation = nn.Tanh() if getattr(config, 'summary_activation', None) == 'tanh' else nn.Identity()
        self.first_dropout = nn.Dropout(getattr(config, 'summary_first_dropout', 0.0))
    ...
    def forward(self, hidden_states, cls_index=None):
        if self.summary_type == 'last':
            ...
            if cls_index is not None:
                output = hidden_states.gather(1, cls_index.unsqueeze(-1).expand(-1, -1, hidden_states.size(-1))).squeeze(1)
            else:
                output = hidden_states[:, -1]
        elif self.summary_type == 'attn':
            # Self-attention over the sequence
            output, _ = self.summary(hidden_states, hidden_states, hidden_states)
            output = output.mean(dim=1)
        elif self.summary_type == 'attn_proj':
            output, _ = self.summary(hidden_states, hidden_states, hidden_states)
            output = self.summary_proj(output.mean(dim=1))
        else:
            output = hidden_states[:, -1]
