...
def safe_getattr(obj, attr_name, default=None):
    """
    安全地获取对象属性，兼容不同版本的transformers
    Safely get object attribute, compatible with different transformers versions
    """
...
try:
    ...
    # 为缺失的函数提供备用实现
    def _crop_past_key_values(past_key_values, max_length):
        """备用实现：裁剪past_key_values"""
        ...
        for layer_past in past_key_values:
            if isinstance(layer_past, tuple):
                # 裁剪每个层的key和value
                cropped_layer = tuple(
                    tensor[..., :max_length, :] if tensor is not None else None
                    for tensor in layer_past
                )
                cropped_past.append(cropped_layer)
            else:
                cropped_past.append(layer_past)
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        # 兼容性检查：forced_decoder_ids在新版本transformers中可能不存在
        forced_decoder_ids = safe_getattr(generation_config, 'forced_decoder_ids', None)
